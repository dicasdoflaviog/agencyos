...
const STATUS_CONFIG: Record<Invoice['status'], { label: string; className: string }> = {
  ...
  paid:      { label: 'Pago',       className: 'bg-[var(--color-success)]/10 text-[var(--color-success)]' },
  overdue:   { label: 'Atrasado',   className: 'bg-[var(--color-error)]/10 text-[var(--color-error)]' },
  cancelled: { label: 'Cancelado',  className: 'bg-white/[0.06] text-[var(--color-text-muted)]' },
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">Faturas</h3>
        <Button
          size="sm"
          onClick={() => setShowForm((v) => !v)}
          className="h-7 gap-1.5 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-[var(--color-text-inverse)] font-semibold text-xs"
        >
      ...
      {showForm && (
        <form
          ...
          className="mb-4 rounded-md border border-[var(--color-accent)]/20 bg-[var(--color-accent)]/[0.04] p-4 space-y-4"
        >
          <p className="text-xs font-semibold text-[var(--color-accent)] uppercase tracking-wide">Nova Fatura</p>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label htmlFor="inv-amount" className="text-xs text-[var(--color-text-secondary)]">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="inv-due" className="text-xs text-[var(--color-text-secondary)]">
                Vencimento <span className="text-[var(--color-error)]">*</span>
              </Label>
              <Input
                id="inv-due"
                type="date"
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <Label htmlFor="inv-notes" className="text-xs text-[var(--color-text-secondary)]">Observações</Label>
              <Textarea
                id="inv-notes"
                rows={2}
                value={notes}
      ...
      {invoices.length === 0 ? (
        <div className="rounded-md border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] p-8 text-center">
          <p className="text-sm text-[var(--color-text-muted)]">Nenhuma fatura encontrada.</p>
          <p className="mt-1 text-xs text-[var(--color-text-disabled)]">Clique em "Nova Fatura" para começar.</p>
        </div>
      ) : (
        <div className="rounded-md border border-[var(--color-border-subtle)] overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border-subtle)] bg-white/[0.02]">
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Valor</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Vencimento</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Pago em</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.04]">
              {invoices.map((invoice) => {
                ...
                return (
                  <tr key={invoice.id} className="bg-[var(--color-bg-surface)] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      {invoice.status !== 'paid' && invoice.status !== 'cancelled' && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded px-2 py-1 text-[11px] font-medium text-[var(--color-success)] hover:bg-[var(--color-success)]/10 disabled:opacity-50 transition-colors"
                        >
