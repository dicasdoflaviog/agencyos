...
export function VulcanStudio({ clientId, clientName, initialJobs = [] }: VulcanStudioProps) {
  ...
  const statusConfig: Record<JobStatus, { label: string; color: string }> = {
    pending:    { label: 'Na fila',    color: 'text-[var(--color-text-secondary)]' },
    processing: { label: 'Gerando...', color: 'text-[var(--color-accent)]' },
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] rounded-xl p-5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-[var(--color-text-primary)]">VULCAN</p>
            <p className="text-xs text-[var(--color-text-secondary)]">Agente Cinematográfico · {clientName}</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {FORMATS.map(f => (
              <button key={f.value} onClick={() => setFormat(f.value)}
                className={`flex flex-col items-center gap-1 p-2.5 rounded-lg border text-xs font-medium transition-all ${format === f.value ? 'border-purple-500/40 bg-purple-500/10 text-purple-400' : 'border-[var(--color-border-subtle)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}>
                <span>{f.icon}</span>
                <span>{f.label}</span>
        ...
        <div>
          <p className="text-xs text-[var(--color-text-secondary)] mb-2">Duração</p>
          <div className="flex gap-2">
            {DURATIONS.map(d => (
              <button key={d} onClick={() => setDuration(d)}
                className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${duration === d ? 'bg-purple-500/20 text-purple-400 border border-purple-500/40' : 'bg-[var(--color-bg-elevated)] text-[var(--color-text-secondary)] border border-transparent'}`}>
                <Clock size={10} />{d}s
              </button>
            ))}
          </div>
        ...
        <div>
          <p className="text-xs text-[var(--color-text-secondary)] mb-2">Descreva o vídeo</p>
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={3}
            placeholder="Ex: Produto de skincare em close, mãos suaves aplicando o creme, iluminação suave, música lo-fi..."
            className="w-full resize-none rounded-lg bg-[var(--color-bg-base)] border border-[var(--color-border-subtle)] px-3 py-2.5 text-sm text-[var(--color-text-primary)] placeholder-[#52525B] focus:outline-none focus:border-purple-500/40 transition-colors" />
        </div>
      ...
      <div className="space-y-3">
        <p className="text-xs text-[var(--color-text-secondary)]">Vídeos gerados</p>
        {jobs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] rounded-xl gap-3">
            <Film size={32} className="text-[var(--color-text-disabled)]" />
            <p className="text-sm text-[var(--color-text-muted)]">Seus vídeos aparecem aqui</p>
          </div>
        ) : (
          jobs.map(j => (
            <div key={j.id} className="bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] rounded-xl p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-[var(--color-text-primary)] truncate">{j.prompt}</p>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-[var(--color-text-secondary)]">{j.format} · {j.duration_s}s</span>
                ...
                {j.status === 'done' && j.video_url && (
                  <div className="flex gap-2 shrink-0">
                    <a href={j.video_url} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1 px-2 py-1 rounded bg-[var(--color-bg-elevated)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] text-xs">
                      <Play size={12} /> Play
                    </a>
